...
const App = () => {
    ...
    const getList= () => {
        axios({
            url:"https://demo-node.cleverapps.io/api/student",
            method : "GET"
        }).then((res)=>{
            var data = res.data;
            setList(data.list)
    ...
    return (
        <SafeAreaView>
            <View style={styles.header_container}>
                <Text style={styles.txt_main}>Course {list}</Text>
                <TouchableOpacity
                ...
                {list && list.map((item,index)=>{
                    return(
                        <View style={styles.item_course} key={index}>
                            <View>
                                <Text style={styles.txt_name}>{index+1}. {item.firstname}-{item.lastname}</Text>
                                <Text style={styles.txt_item}>{item.gender == 1 ? "Male" : "Female"}</Text>
